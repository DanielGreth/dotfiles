## Basic Options
	unsetopt beep notify		# Disable error beep
	unsetopt global_rcs		# Skip reading global configs for interactive usage
	setopt complete_in_word		# Complete from both ends of a word
	setopt always_to_end		# Move cursor to the end of a completed word
	setopt auto_cd			# 'dirname' is equal to 'cd dirname'
	...
	setopt prompt_subst		# Command substitution, parameters and arithmetic expansion in prompt
...
# Change title to command name
	function title {
		if [[ -n ${terminfo[fsl]} && -n ${terminfo[ts1]} ]]; then
			echoti ts1; print -Pn "$1"; echoti fs1
		fi
	}
